  ...
  return assetYearIncomes(data, now)
    ...
    .map((r) => {
      ...
      return {
        ...
        tax: r.annual * rate,
        // Факт «на сегодня» — ТОЛЬКО для бессрочных. У срочного набежавшее не
        // соответствует ничему реальному: забрать эти деньги посреди срока без
        // потери процентов нельзя, поэтому показывать «уже набежало» нечестно.
        // У бессрочного наоборот: снял в любой день — банк начислил на эту
        // сумму, и её надо знать. У срочного значима одна цифра — итоговая,
  ...
  for (const v of views) {
    ...
    if (isMatured) {
      ...
      if (idleDays >= longestDays) {
        ...
        longestAssetId = v.asset.id;
      }
      continue;
    }
    if (v.derived.incomePerDay > 0) {
      // Взвешиваем НОМИНАЛЬНУЮ ставку, а не «доход / текущая стоимость»:
      // накопленные проценты раздувают знаменатель и занижают её, а
      // простаивающие деньги разместят именно под номинальную.
      earningCapital += cap;
      earningWeightedRate += v.derived.currentRate * cap;
    }
  }
