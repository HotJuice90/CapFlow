...
const semantic = {
  ...
  // Светлый «рост» для приглушённых состояний (неактивные столбики графика).
  // Именно СПЛОШНОЙ цвет, а не positive под alpha: полупрозрачный насыщенный
  // зелёный подмешивает синеватый фон карточек (#F9FAFF) и уходит в пыльный
  // серо-зелёный. Взят как середина между positive и светлой точкой градиента
  // роста (#2CE296) — тот же тон, просто чище и легче.
  //
  // Осветлять дальше (пробовали #8FE3C0, за верхней точкой градиента) нельзя:
  // зелёный уходит в бледно-неоновый и проваливается в фон. Отличие активного
  // столбика держится не светлотой соседей, а самим фактом градиента.
  positiveLight: '#45CE95',
  negative: '#E5484D',
  warning: '#F2A900',
  info: '#3E63DD',
};
