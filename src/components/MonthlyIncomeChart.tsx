...
/**
 * Неактивные столбики — базовый цвет под низкой alpha. Подбор сплошного
 * светлого оттенка сюда не годится: активный столбик залит градиентом, и любой
 * сплошной зелёный либо попадает внутрь его диапазона и сливается (#45CE95),
 * либо уходит светлее верхней точки и становится бледно-неоновым (#8FE3C0) —
 * обе стороны проверены на живом. Плотные значения alpha (0.7) при этом дают
 * пыльный серо-зелёный: насыщенный цвет подмешивает синеватый фон карточки.
 * 0.4 — та же логика, что у подложек-плашек в приложении: цвет читается как
 * лёгкая заливка, а не как «выцветший столбик».
 */
        ...
        {data.months.map((m, i) => {
          ...
          return (
            <Pressable
              key={m.month}
              style={styles.col}
              // Хаптик только на РЕАЛЬНОМ переключении: тап по уже выбранному
              // месяцу ничего не меняет, и отклик на него читался как отклик
              // «на пустоту».
              onPress={() => {
                if (i === idx) return;
                tapBuzz();
