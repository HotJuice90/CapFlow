...
export function CapitalAxisChart({
  ...
  //
  // Точка отсчёта сетки — от РЕАЛЬНОГО rawMin, а не «уширенного» effectiveMin
  // (тот только выбирает шаг, чтобы плоский участок не зумило до рублей):
  // иначе появлялась лишняя строка снизу, ниже которой линия графика вообще
  // никогда не опускается. Пол НЕ клампим в 0 сразу — иначе сетка теряет фазу
  // (0/2/4/6 вместо 1/3/5/7) и потолок перестаёт попадать в шаг от низа.
  // Клампим только на этапе фильтра видимых линий.
  const niceMinRaw = Math.floor((rawMin - half) / step) * step + half;
  const niceMax = Math.ceil((rawMax - half) / step) * step + half;
  const gridValues: number[] = [];
  for (let v = niceMinRaw; v <= niceMax + step * 0.01; v += step) gridValues.push(v);
  // Ноль — честная нижняя граница масштаба (когда реальный минимум ряда
  // близко к нулю), но отдельной линией/подписью его не рисуем: и так
  // понятно, что низ графика — примерно ноль, а сама подпись «0» шумит.
  ...
  const axisMin = Math.max(0, niceMinRaw - step * 0.25);
