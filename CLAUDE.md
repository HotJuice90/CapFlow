...
- **Дальше по размеру, если понадобится**: R8 выключен (`enableMinifyInReleaseBuilds`),
  ...
  ломается всё, что ходит через рефлексию. Настоящее решение «не качать всё ради
  правки экрана» — `expo-updates` с self-hosted манифестом (без EAS): JS-only
  правки весили бы единицы мегабайт, но нативные (новый модуль, разрешение)
  всё равно требуют полного APK, т.е. останутся два канала.
