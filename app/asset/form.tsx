...
export default function AssetFormScreen() {
  ...
  const askAmountChangeKind = (asset: Asset) => {
    // Числа в тексте — не украшение: без них выбор читается наоборот. Легко
    // думать «ввожу правильную изначальную сумму», выбрать «деньги двигались»
    // и получить рост баланса вместо снятия. Показываем оба значения и что
    // именно случится в каждом варианте.
    const was = formatMoney(initial.amount ?? asset.amount, { currency, kopecks: 'hide' });
    const now = formatMoney(asset.amount, { currency, kopecks: 'hide' });
    appAlert(
      'Сумма изменилась',
      `Было ${was} → ввели ${now}.\n\n` +
        `«Так было всегда» — исправление: актив с самого открытия был на ${now}.\n\n` +
        `«Деньги двигались» — до выбранной даты остаётся ${was}, а с неё баланс становится ${now}.`,
      [
        { text: 'Отмена', style: 'cancel' },
