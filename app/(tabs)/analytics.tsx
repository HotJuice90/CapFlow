...
Расчёт обновится только в том случае, если изменятся условия актива, например при досрочном закрытии."
            ...
            {/* Ликвидность — сколько можно снять прямо сейчас без потери %
                (бессрочные счета) против того, что заморожено до срока
                (вклады/облигации/ЦФА). Показываем, только если в портфеле
                вообще есть срочные активы — иначе бар из одного сегмента
                бессмысленный. */}
            {liq.frozen > 0 ? (
              <>
                ...
                <View style={styles.liqCard}>
                  ...
                  <View style={[styles.taxMeta, { justifyContent: 'flex-end' }]}>
                    <View style={styles.taxMetaInlineRow}>
                      ...
                      >
                        <View style={styles.liqRing}>
                          <Donut
                            ...
                            strokeWidth={4.5}
                          />
                          <View style={styles.liqLockCircle}>
...
const styles = StyleSheet.create({
  ...
  liqLockCircle: { position: 'absolute', width: 22, height: 22, borderRadius: 11, backgroundColor: tokens.surface.white, alignItems: 'center', justifyContent: 'center' },
